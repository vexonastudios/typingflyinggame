...
class DuckHuntDuel {
  ...
  _claimTarget(target, who) {
    ...
    if (target.type === 'golden') { Sfx.hitBig(); this.shake = 20; } else Sfx.hit();
    
    spawnBurst(this.particles, target.px, target.py, col, 14, target.type==='golden'?360:240);
  ...
  _updateHUD() {
    ['p1', 'p2'].forEach(id => {
      ...
      document.getElementById(`score${id==='p1'?'P1':'P2'}`).textContent = p.score;
      const comboEl = document.getElementById(`combo${id==='p1'?'P1':'P2'}`);
      comboEl.textContent = p.combo > 1 ? `×${Math.min(p.combo,8)} COMBO` : '';
  ...
  _draw() {
    ...
    for (const tgt of this.targets) { if (tgt.deadTimer < 0.35) { ctx.globalAlpha = tgt.dead ? Math.max(0, 1 - tgt.deadTimer * 4) : 1; this._drawTarget(ctx, tgt, t); } }
    ctx.globalAlpha = 1;
    ...
    ctx.restore();
    if (this.state === 'playing' || this.state === 'waveclear') { this._drawTurret(ctx, this.p1, COLORS.p1); if (this.mode === 'versus') this._drawTurret(ctx, this.p2, COLORS.p2); }
    ctx.restore();
  }
  ...
  _drawTurret(ctx, p, col) {
    ...
    ctx.fillStyle = 'rgba(0,0,0,0.3)'; ctx.beginPath(); ctx.ellipse(0, 0, 35, 12, 0, 0, Math.PI*2); ctx.fill();
    ctx.save(); ctx.translate(0, -12); ctx.rotate(p.angle); ctx.translate(-p.recoil, 0);
    ctx.fillStyle = col; ctx.fillRect(0, -10, 60, 20); ctx.fillStyle = 'rgba(255,255,255,0.2)'; ctx.fillRect(0, -10, 60, 6);
    ctx.fillStyle = '#222'; ctx.fillRect(35, -11, 20, 22); ctx.fillStyle = '#111'; ctx.fillRect(60, -12, 8, 24);
    ctx.restore();
    ctx.fillStyle = '#1a1a1a'; ctx.beginPath(); ctx.arc(0, -5, 30, Math.PI, 0); ctx.fill();
    ctx.fillStyle = col; ctx.beginPath(); ctx.moveTo(-35, 0); ctx.lineTo(-25, -18); ctx.lineTo(25, -18); ctx.lineTo(35, 0); ctx.fill();
    ctx.restore();
  }
}
